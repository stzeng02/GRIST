...
x = tf.placeholder(tf.float32, shape=[None, 1024]) # 784 -> 1024
y_ = tf.placeholder(tf.float32, shape=[None, 10])

W_conv1 = weight_variable([5, 5, 1, 32])
b_conv1 = bias_variable([32])

x_image = tf.reshape(x, [-1, 32, 32, 1]) # 28, 28 -> 32, 32
...
W_fc1 = weight_variable([4096, 1024]) # 3136 / (28 * 28) = 4 -> 4 * (32 * 32) = 4096
b_fc1 = bias_variable([1024])

h_pool2_flat = tf.reshape(h_pool2, [-1, 4096]) # 3136 / (28 * 28) = 4 -> 4 * (32 * 32) = 4096
...
gradient_search = GradientSearcher(name="sosip2_grist")
