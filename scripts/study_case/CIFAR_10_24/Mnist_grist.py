...
with tf.name_scope('input'):
    x = tf.placeholder(tf.float32, shape=[None, 1024],name='x-input') # 784 -> 1024
    y_label = tf.placeholder(tf.float32, shape=[None, 10],name='y-input')

with tf.name_scope('input_reshape'):
    x_reshape = tf.reshape(x, [-1, 32, 32, 1]) # 28, 28 -> 32, 32
...
# Layer3
w_fc1 = weight([4096, 1024]) # 3136 / (28 * 28) = 4 -> 4 * (32 * 32) = 4096
b_fc1 = bias([1024])
