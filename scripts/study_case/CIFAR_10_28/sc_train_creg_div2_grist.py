...
l1_matmul = {
    ...
    'gain': tf.get_variable(name="G_matmul_1",
                            initializer=tf.random_uniform([n_hidden], minval=0, maxval=16), trainable=False)
}
...
lo_matmul = {
    ...
    'gain': tf.get_variable(name="G_matmul_2",
                            initializer=tf.random_uniform([n_classes], minval=0, maxval=16), trainable=True)
}
...
with tf.Session() as sess:
    ...
    scheduler = TensorFlowScheduler(name="SC_DNN.sc_train_creg_div2_grist")
    '''inserted code'''
    step = 0
