...
z = tf.add(z_mean, tf.multiply(tf.sqrt(tf.exp(tf.clip_by_value(z_log_sigma_sq, -87, 87))), eps))
...
latent_loss = -0.5 * tf.reduce_sum(1 + z_log_sigma_sq - tf.square(z_mean) - tf.exp(tf.clip_by_value(z_log_sigma_sq, -87, 87)), 1)
...
def train(sess, batch_size=100, training_epochs=10, display_step=5):
    ...
    gradient_search = GradientSearcher(name="ch10_04_01_grist")
    ...
    while True:
        ...
        for i in range(total_batch):
            ...
            _, loss_val, r_loss, l_loss = sess.run((optimizer, cost, reconstr_loss_mean, latent_loss_mean), feed_dict=feed_dict)
